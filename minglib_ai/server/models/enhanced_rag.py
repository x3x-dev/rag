"""
Enhanced RAG for FastAPI - Based on enhanced_llamaindex.py
...
class MingLibRAG:
    def __init__(
        ...
        self.debug_handler = LlamaDebugHandler(print_trace_on_end=False)  # Set to False for production
    ...
    def initialize(self, force_rebuild: bool = False, retrieval_mode: str = "hybrid") -> None:
        logger.info("Initializing MingLib Enhanced RAG...")
        self.setup_models()
        docs = self.load_documents()
        self.create_index(docs, force_rebuild=force_rebuild)
        self.setup_query_engine(retrieval_mode=retrieval_mode)
        logger.info("MingLib Enhanced RAG initialized")

    # ----- Query API -----
    def query(self, question: str, retrieval_mode: str = "hybrid") -> str:
...
# ---------- Global RAG Instance ----------
_rag_instance: Optional[MingLibRAG] = None


# ---------- Public API Functions ----------
def init_enhanced_rag(force_rebuild: bool = False) -> None:
    """Initialize the enhanced RAG system. Call once at startup."""
    global _rag_instance
    
    if _rag_instance is not None and not force_rebuild:
        logger.info("Enhanced RAG already initialized; skipping.")
        return
    ...
    persist_dir = current_file.parent / "storage"
    
    _rag_instance = MingLibRAG(
    ...
    _rag_instance.initialize(force_rebuild=force_rebuild, retrieval_mode="hybrid")
    logger.info("Enhanced RAG initialized for FastAPI")


def get_enhanced_response(question: str) -> dict:
    """
    Get response from enhanced RAG system.
    Safe for request-time use. If init was skipped, lazily initializes once.
    """
    global _rag_instance
    
    if _rag_instance is None:
        # Lazy initialization if startup hook didn't run
        init_enhanced_rag()
    
    start = time.time()
    try:
        content = _rag_instance.query(question, retrieval_mode="hybrid")
        return parse_response(content, start, model_type="enhanced")
    except Exception as e:
        logger.exception("Enhanced RAG request failed")
        return {
            "success": False,
            "error": {
                "message": str(e),
                "code": "PROCESSING_ERROR",
                "details": {"model_type": "enhanced", "question": question},
            },
        }